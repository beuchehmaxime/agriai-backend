...
export class MessageService {
    async getMessages(consultationId: string, userId: string, page: number = 1, limit: number = 50) {
    ...
    async sendMessage(senderId: string, consultationId: string, text?: string, imageUrl?: string) {
        const consultation = await prisma.consultation.findUnique({
            where: { id: consultationId }
        });
