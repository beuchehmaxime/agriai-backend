...
export class SubscriptionService {
    ...
    async purchasePlan(farmerId, planId, quantity, phoneNumber, provider) {
        if (quantity < 1)
    ...
    async getMySubscriptions(farmerId) {
        return this.repository.findByFarmerId(farmerId);
    }
    async getSubscribers(agronomistId) {
        return this.repository.findSubscribers(agronomistId);
    }
    async deductHours(agronomistId, subscriptionId, hoursToDeduct) {
        if (hoursToDeduct <= 0)
            throw new Error('Hours to deduct must be greater than 0');
        ...
        if (sub.plan.agronomistId !== agronomistId)
            throw new Error('Unauthorized to manage this subscription');
        if (sub.isUnlimited) {
