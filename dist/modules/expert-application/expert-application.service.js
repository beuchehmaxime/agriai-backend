...
export class ExpertApplicationService {
    async submitApplication(userId, experience, qualifications, certificateType, obtainedYear, institution, certificateUrl) {
        ...
        if (user.userType === 'AGRONOMIST')
            throw new Error('User is already an agronomist');
        ...
        if (rejectedCount >= 4)
            throw new Error('Maximum application limit (4) reached');
