...
import { requireActiveSubscriptionForMessage } from '../subscription/subscription.middleware.js';
const router = Router();
// Uses upload.single('image') to support image uploads
router.get('/:consultationId', authenticate, controller.getMessages);
router.post('/:consultationId', authenticate, requireActiveSubscriptionForMessage, upload.single('image'), controller.sendMessage);
